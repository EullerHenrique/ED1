...
LIST *list_create(int type_list){
  LIST *L = NULL;
...
int list_pop_front(LIST *L){
  ...
  //Se a lista não for vazia
    ...
    if(L -> size == 1){
      free(L -> head); // O primeiro nó é removido
      L -> head = NULL;
    }
...
int list_pop_pos(LIST *L, int pos){
  ...
  //Se a lista não for vazia
    ...
    if(L -> size == 1){
      free(L -> head); // Libera a memória do primeiro nó
      
      L -> head = NULL; // O primeiro nó fica vazio. Apaga o endereço do primeiro nó, e consequentemente seus dados
    }
...
int list_size(LIST *L){
  ...
  return L -> size; // retorn o tamanho da lista
}  
